...
use RS_Vendor\Twig\Extension\GlobalsInterface;
use RS_Vendor\Twig\Extension\InitRuntimeInterface;
...
final class ExtensionSet
{
    ...
    {
        $this->staging = new \RS_Vendor\Twig\Extension\StagingExtension();
    }
    /**
     ...
     */
    public function initRuntime(\RS_Vendor\Twig\Environment $env)
    {
        ...
        foreach ($this->extensions as $extension) {
            if ($extension instanceof \RS_Vendor\Twig\Extension\InitRuntimeInterface) {
    ...
    public function getExtension(string $class) : \RS_Vendor\Twig\Extension\ExtensionInterface
    {
        $class = \ltrim($class, '\\');
        if (!isset($this->extensions[$class]) && \class_exists($class, \false)) {
            // For BC/FC with namespaced aliases
        ...
        if (!isset($this->extensions[$class])) {
            throw new \RS_Vendor\Twig\Error\RuntimeError(\sprintf('The "%s" extension is not enabled.', $class));
        }
    ...
    public function getGlobals() : array
    {
        ...
        $globals = [];
        foreach ($this->extensions as $extension) {
            if (!$extension instanceof \RS_Vendor\Twig\Extension\GlobalsInterface) {
                continue;
            }
    ...
    private function initExtension(\RS_Vendor\Twig\Extension\ExtensionInterface $extension)
    {
        // filters
        ...
        foreach ($extension->getTokenParsers() as $parser) {
            if (!$parser instanceof \RS_Vendor\Twig\TokenParser\TokenParserInterface) {
...
\class_alias('RS_Vendor\\Twig\\ExtensionSet', 'RS_Vendor\\Twig_ExtensionSet');
