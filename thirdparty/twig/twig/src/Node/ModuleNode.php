 ...
 */
class ModuleNode extends \RS_Vendor\Twig\Node\Node
{
    public function __construct(\RS_Vendor\Twig\Node\Node $body, \RS_Vendor\Twig\Node\Expression\AbstractExpression $parent = null, \RS_Vendor\Twig\Node\Node $blocks, \RS_Vendor\Twig\Node\Node $macros, \RS_Vendor\Twig\Node\Node $traits, $embeddedTemplates, \RS_Vendor\Twig\Source $source)
    {
        ...
        $nodes = ['body' => $body, 'blocks' => $blocks, 'macros' => $macros, 'traits' => $traits, 'display_start' => new \RS_Vendor\Twig\Node\Node(), 'display_end' => new \RS_Vendor\Twig\Node\Node(), 'constructor_start' => new \RS_Vendor\Twig\Node\Node(), 'constructor_end' => new \RS_Vendor\Twig\Node\Node(), 'class_end' => new \RS_Vendor\Twig\Node\Node()];
        if (null !== $parent) {
            $nodes['parent'] = $parent;
        }
    ...
    protected function compileTemplate(\RS_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileGetParent(\RS_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassHeader(\RS_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("\n\nnamespace RS_Vendor;\n\n");
        if (!$this->getAttribute('index')) {
            $compiler->write("use \\RS_Vendor\\Twig\\Environment;\n")->write("use \\RS_Vendor\\Twig\\Error\\LoaderError;\n")->write("use \\RS_Vendor\\Twig\\Error\\RuntimeError;\n")->write("use \\RS_Vendor\\Twig\\Extension\\SandboxExtension;\n")->write("use \\RS_Vendor\\Twig\\Markup;\n")->write("use \\RS_Vendor\\Twig\\Sandbox\\SecurityError;\n")->write("use \\RS_Vendor\\Twig\\Sandbox\\SecurityNotAllowedTagError;\n")->write("use \\RS_Vendor\\Twig\\Sandbox\\SecurityNotAllowedFilterError;\n")->write("use \\RS_Vendor\\Twig\\Sandbox\\SecurityNotAllowedFunctionError;\n")->write("use \\RS_Vendor\\Twig\\Source;\n")->write("use \\RS_Vendor\\Twig\\Template;\n\n");
        }
    ...
    protected function compileConstructor(\RS_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileDisplay(\RS_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassFooter(\RS_Vendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('class_end'))->outdent()->write("}\n");
    }
    protected function compileMacros(\RS_Vendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('macros'));
    }
    protected function compileGetTemplateName(\RS_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getTemplateName()\n", "{\n")->indent()->write('return ')->repr($this->getSourceContext()->getName())->raw(";\n")->outdent()->write("}\n\n");
    }
    protected function compileIsTraitable(\RS_Vendor\Twig\Compiler $compiler)
    {
        ...
        if ($traitable) {
            if ($this->getNode('body') instanceof \RS_Vendor\Twig\Node\BodyNode) {
                $nodes = $this->getNode('body')->getNode(0);
            } else {
                $nodes = $this->getNode('body');
            }
            if (!\count($nodes)) {
                $nodes = new \RS_Vendor\Twig\Node\Node([$nodes]);
            }
            foreach ($nodes as $node) {
                ...
                if ($node instanceof \RS_Vendor\Twig\Node\BlockReferenceNode) {
                    continue;
                }
                $traitable = \false;
                break;
    ...
    protected function compileDebugInfo(\RS_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getDebugInfo()\n", "{\n")->indent()->write(\sprintf("return %s;\n", \str_replace("\n", '', \var_export(\array_reverse($compiler->getDebugInfo(), \true), \true))))->outdent()->write("}\n\n");
    }
    protected function compileGetSourceContext(\RS_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileLoadTemplate(\RS_Vendor\Twig\Compiler $compiler, $node, $var)
    {
...
\class_alias('RS_Vendor\\Twig\\Node\\ModuleNode', 'RS_Vendor\\Twig_Node_Module');
